...
def create_and_evaluate_model(complexcode):
    ...
    plt.scatter(combined_df['elec_usage'], combined_df['Forecast'], color='blue', alpha=0.5)
    plt.plot([combined_df['elec_usage'].min(), combined_df['elec_usage'].max()],
             [combined_df['elec_usage'].min(), combined_df['elec_usage'].max()],
             color='red', linewidth=2)
    plt.xlabel('Actual Electricity Usage')
    plt.ylabel('Forecasted Electricity Usage')
    plt.title('Actual vs Forecasted Electricity Usage')
    plt.show()
